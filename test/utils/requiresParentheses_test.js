...
withBuiltLibrary('utils/requiresParentheses', function(required) {
  requiresParentheses = required;
});
...
describe('requiresParentheses', function() {
  it('is false for identifiers', function() {
    ...
    assert.strictEqual(requiresParentheses(node), false);
  });

  it('is true for binary plus expressions', function() {
    const node = parse('a + b').body.statements[0];
    assert.strictEqual(requiresParentheses(node), true);
  });
