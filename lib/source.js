...
function sourceOffsetForCSNodeEnd(source, csNode) {
  ...
  if (csNode.properties && csNode.properties.length > 0) {
    endNode = csNode.properties[csNode.properties.length - 1];
  }

  return sourceOffsetForLineAndColumn(
    source,
    endNode.locationData.last_line,
    endNode.locationData.last_column
  ) + 1;
}
 ...
 * @param {number} start
 * @param {number} end
 * @returns {{start: number, end: number, string: string}}
 */
function rangeOfLongestBalancedString(source, start, end) {
  var openParenCount = 0;
  var openBracketCount = 0;
  var openBraceCount = 0;
  var realEnd = start;
  var lastBalancedOffset = start;

  while (realEnd < source.length) {
    switch (source[realEnd]) {
      case '(':
        openParenCount++;
        break;
    ...
    if (openParenCount === 0 && openBracketCount === 0 && openBraceCount === 0) {
      lastBalancedOffset = realEnd + 1;
      if (lastBalancedOffset >= end) {
        break;
      }
    }
  ...
  return {
    start: start,
    end: lastBalancedOffset,
    string: source.slice(start, lastBalancedOffset)
  };
}
exports.rangeOfLongestBalancedString = rangeOfLongestBalancedString;
