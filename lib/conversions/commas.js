...
/**
 * Returns splices necessary to ensure the given list has no optional commas.
 *
 * @param {string} source
 * @param {Array} list
 * @returns {Splice[]}
 */
function getCommaSplicesForList(source, list) {
  const splices = [];
  ...
  listWithoutComments.forEach(function(obj, i) {
    ...
    if (trailingWhitespaceMatch) {
      ...
      splices.push({
        start: expectedCommaIndex,
        end: expectedCommaIndex,
        value: '\n' + indentWhitespace + ','
      });
    } else {
...
exports.getCommaSplicesForList = getCommaSplicesForList;
